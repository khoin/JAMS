...
var AudioContext = (AudioContext !== undefined) ? AudioContext : webkitAudioContext;
...
var JAMS = function(config) {
	...
	this.cmx = 0;
	this.cmy = 0;
	// Workspace translation/scale
	this.cx = 0;
	this.cy = 0;
	this.sx = this.interface.scale = 1;
	this.sy = this.interface.scale = 1;
	...
	this.processor = this.aC.createScriptProcessor(0, 0, 1);
	this.t = 0;
	...
	this.plugins = [{
		...
		children: ["JAMSt", "JAMSNote2Freq","JAMSMilliseconds"]
	},
	{
		name: "Filters",
		children: ["JAMSOnePole"]
	},
	{
		name: "Envelopes",
	...
	{
		name: "Generators",
		children: ["JAMSSineGenerator", "JAMSSawtoothGenerator"]
	},
	{
		name: "MIDI",
...
JAMS.prototype.init = function() {
	...
	// zooming
	_.display.DOMElement.addEventListener("wheel", e => { e.preventDefault();
		if(e.ctrlKey) {
			_.sx -= e.deltaY/1000; _.sx = _.interface.scale = Math.round(_.sx*10)/10;
			_.sy -= e.deltaY/1000; _.sy = Math.round(_.sy*10)/10;
		}
	})
	...
	_.display.DOMElement.addEventListener("mousemove", e => {
		_.cmx = e.clientX/_.sx;
		_.cmy = e.clientY/_.sy;
		_.interface.handleMove(_.cmx, _.cmy);
	});
	
	_.display.DOMElement.addEventListener("touchmove", e => {
		_.cmx = e.touches[0].clientX/_.sx;
		_.cmy = e.touches[0].clientY/_.sy;
		_.interface.handleMove(_.cmx, _.cmy);
	});
	...
	this.display.DOMElement.addEventListener("contextmenu", e => {
		e.preventDefault();
		let mx = e.clientX/_.sx - _.cx;
		let my = e.clientY/_.sy - _.cy;
		...
		var mouseOnMod = _.modules.some( module => {
			if( mx > module.x - _.ioDisplayWidth && mx < module.x + module.width + _.ioDisplayWidth && my > module.y && my < module.y+module.height ) {
				...
				if( mx < module.x && my < module.y + module.numberOfInputs*_.ioDisplayHeight ) {
					...
					_.interface.createContextMenu(e.clientX/_.sx, e.clientY/_.sy, {padding:4}, options);
					return true;
				}
					...
					},
					{
						...
						callback: function(x,y) {
							...
							_.interface.createWindow({
								...
								x: x, y: y,
								children: [
									Interface.TextNode({content: "", width: 0.1, height: 1}),
									Interface.TextNode({content: module.helpText, width: 0.9, height: 1, xAnchor: "left"})
								]
							})
				...
				if( module !== _.outputModule) _.interface.createContextMenu(e.clientX/_.sx, e.clientY/_.sy, {padding:4}, options);
				return true;
			}	
		});
	...
	function mouseDownHandler(e) {
		if(e.touches) { e.clientX = e.touches[0].clientX; tmx = e.touches[0].clientX/_.sx; e.clientY = e.touches[0].clientY; tmy = e.touches[0].clientY/_.sy; }
		// Let the interface handles clicks
		var isInterfaceBlocking = _.interface.handleClick(e.clientX/_.sx, e.clientY/_.sy, _.display.DOMElement);
	...
	function mouseUpHandler(e) {
		...
		if(e.changedTouches) { e.clientX = e.changedTouches[0].clientX; e.clientY = e.changedTouches[0].clientY; }
		let mx = (e.clientX/_.sx - _.cx); 
		let my = (e.clientY/_.sy - _.cy);
	...
	function rightClickOnBackground(e) {
		let mx = e.clientX/_.sx;
		let my = e.clientY/_.sy;
		...
		var options = [{
			...
			text: "new setup",
			callback: () => {
				let win = _.interface.createWindow({
					title: "Confirm", x: mx, y: my, width: 200, height: 50,
					resizable: false,
					children: [
						Interface.TextNode({ content: "Are you sure you\nwant to clear the setup?", height: 0.5, width: 1}),
						Interface.Button({
							content: "Clear it!", width: 0.5, height: 0.5,
							onClick: () => { _.loadSetup('[{"id":1,"type":"JAMSOutput","x":516,"y":275,"inputs":[],"params":[],"name":"output"}]'); _.interface.closeWindow(win);}
						}),
						Interface.Button({
							content: "No! Cancel", width:0.5, height: 0.5,
							onClick: () => { _.interface.closeWindow(win); }
						})
					]
				})
			}
			...
			callback: () => {
				_.interface.createWindow({
					title: "about JAMS", x: mx, y: my, width: 240, height: 100,
					resizable: false,
		...
		_.interface.createContextMenu(mx, my, {padding: 4} , options);
	}

	// module drag
	function mouseDragModuleHandler(e) { 
		if(e.touches) {  
			_.currentModule.x = -_.cx + e.touches[0].clientX/_.sx - _.currentModule.width/2;
			_.currentModule.y = -_.cy + e.touches[0].clientY/_.sy - _.currentModule.height/2;
			return;
		}
		_.currentModule.x += e.movementX/_.sx;
		_.currentModule.y += e.movementY/_.sy;
	}
	//canvas drag
	function mouseDragCanvasHandler(e) {
		if(e.touches) { e.movementX = e.touches[0].clientX/_.sx - tmx; e.movementY = e.touches[0].clientY/_.sy - tmy; 
			tmx=e.touches[0].clientX/_.sx;
			tmy=e.touches[0].clientY/_.sy;
		}
		_.cx += e.movementX/_.sx;
		_.cy += e.movementY/_.sy;
	}

	// shiftDrag means throw the event to module's drag handler
	function mouseShiftDragModuleHandler(e) { 
		if(e.touches) { e.clientX = e.touches[0].clientX/_.sx; e.clientY = e.touches[0].clientY/_.sy; }
	...
	(function loop() {
		...
		_.display.context.scale(_.sx,_.sy);
			_.renderLoop();
...
JAMS.prototype.renderLoop = function(w) {
	...
	_.display.text(0,0,`ScaleX: ${_.sx}\nScaleY: ${_.sy}`);
	_.display.context.translate(Math.floor(_.cx), Math.floor(~~_.cy));
