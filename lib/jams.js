...
JAMS.prototype.init = function() {
						...
						callback: function(x,y) {
							...
							if(module.params.length > 0) {
								module.params.forEach( (param,index,arr) => { 
									c.push(Interface.TextNode({ content: param.name, width: 0.5, height: 1/(arr.length+1) }));
									switch(param.type) {
										case "number":
											c.push(Interface.NumberNode({
												width: 0.5, height: 1/(arr.length+1),
												getValue: () => param.value,
												setValue: x => {param.value = x;}
											}));
										break;
										default:
											c.push(Interface.TextNode({content: "<unsupported type>", width:0.5, height: 1/(arr.length+1)}));
										break;
									}
								})
							}
	...
	function rightClickOnBackground(e) {
		var plugins = ["JAMSNumber", "JAMSSineGenerator", "JAMSMultiplier","JAMSt", 
		"JAMSRemainder", "JAMS4Array", "JAMSmidiNote", "JAMSNote2Freq", "JAMSOnOffEnvelope", "JAMSSimpleDecay", "JAMSPlus"];
