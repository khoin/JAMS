	...
	this.windows.some( (win,index) => { 
		if( x > win.x  && x < win.x + win.width && y > win.y-9 && y < win.y + win.height ) {
			...
			win.children.some( child => {
				...
				if( x > gx && x < gx + child.calcWidth() &&
					y > gy && y < gy + child.calcHeight() ) {
...
Interface.calculateNodeOffset = function(win, node) {
	return {
		ox: (node.config.xAnchor == "center")? (win.width/2 *node.config.width -node.calcWidth() /2) : 0,
		oy: (node.config.yAnchor == "center")? (win.height/2*node.config.height-node.calcHeight()/2) : 0
	}
}
...
Interface.NumberNode.prototype.calcWidth = function() {
	return 56;
}
...
Interface.NumberNode.prototype.handleClick = function(x,y) {
	...
	if( x>8 && x<48 ) {
		this.inputBuffer = (this.config.getValue)? (Math.round(this.config.getValue()*100)/100).toString() : "";
...
Interface.NumberNode.prototype.render = function(ctx, x, y) {
	...
	ctx.context.fillStyle = "#fff";
	ctx.box(48,0,8,8);
	if( x>48 && x<56 && y>0 && y<8 ) { ctx.context.fillRect(48,0,8,8); ctx.context.fillStyle = "#000"; }
	ctx.text(51,2,"+");
	ctx.context.fillStyle = "#fff";
	ctx.box(8,0,40,8);
	if( this.keyboardListening ) { 
		ctx.context.fillRect(8,0,40,8); ctx.context.fillStyle = "#000"; 
		ctx.text(11,2, this.inputBuffer);
	} else {
		ctx.text(11,2,(this.config.getValue)? Math.round(this.config.getValue()*100)/100 : NaN);	
	}
}
